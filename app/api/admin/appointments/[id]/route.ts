...
export async function PATCH(
  ...
  try {
    ...
    if (!Object.values(AppointmentStatus).includes(status as AppointmentStatus)) {
      return NextResponse.json({ error: "Захиалгын төлөв буруу байна." }, { status: 400 });
    }
    const appointment = await prisma.appointment.findUnique({ where: { id }, select: { id: true } });
    if (!appointment) {
    ...
    return NextResponse.json(
      { error: "Шинэчлэхэд алдаа гарлаа", details: error instanceof Error ? error.message : String(error) },
      { status: 500 },
    );
  }
}
...
export async function DELETE(
  ...
  try {
    ...
    return NextResponse.json(
      { error: "Устгахад алдаа гарлаа", details: error instanceof Error ? error.message : String(error) },
      { status: 500 },
    );
  }
}
