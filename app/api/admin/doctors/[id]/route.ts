...
export async function PATCH(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (!input.name) return NextResponse.json({ error: "Эмчийн нэр заавал байна." }, { status: 400 });
    const doctor = await prisma.doctor.update({ where: { id }, data: input });
    return NextResponse.json({ success: true, data: doctor });
  } catch (error) {
    console.error("PATCH /api/admin/doctors/[id] error:", error);
    return NextResponse.json({ error: "Эмчийн мэдээлэл шинэчлэхэд алдаа гарлаа." }, { status: 500 });
  }
}
...
export async function DELETE(_request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (!doctor) return NextResponse.json({ error: "Эмч олдсонгүй." }, { status: 404 });
