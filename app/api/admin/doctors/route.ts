...
export async function GET() {
  try {
    ...
    return NextResponse.json({ error: "Эмчийн мэдээлэл авахад алдаа гарлаа." }, { status: 500 });
  }
}
...
export async function POST(request: Request) {
  try {
    ...
    if (!input.name) {
      return NextResponse.json({ error: "Эмчийн нэр заавал байна." }, { status: 400 });
    }
    ...
    return NextResponse.json({ error: "Эмч нэмэхэд алдаа гарлаа." }, { status: 500 });
  }
}
